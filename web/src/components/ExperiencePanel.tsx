...
import { addFestival, getActivities, getEvents, getNatureSpots, getSouvenirShops } from "../api/client";
import type { ActivitySpot, Festival, ItineraryOutput, SouvenirShop } from "../types";

type DiscoveryTab = "FESTIVAL" | "NIGHT_MARKET" | "TRADITIONAL_MARKET" | "ACTIVITY" | "NATURE" | "SOUVENIR";
const labels: Record<DiscoveryTab, { title: string; description: string; index: string }> = {
  ...
  NATURE: { title: "산책, 자연", description: "해수욕장·산책로·숲길 등 걷고 쉬기 좋은 곳", index: "05" },
  SOUVENIR: { title: "기념품샵", description: "지역의 기억을 담아갈 가까운 상점", index: "06" },
};
...
export function ExperiencePanel({ itinerary, canEdit = true }: { itinerary: ItineraryOutput; canEdit?: boolean }) {
  ...
  const [natureSpots, setNatureSpots] = useState<ActivitySpot[]>([]);
  ...
  useEffect(() => {
    ...
    getSouvenirShops(itinerary.trip.originLat, itinerary.trip.originLng).then(setShops).catch(() => setShops([]));
    getActivities(itinerary.trip.originLat, itinerary.trip.originLng).then(setActivities).catch(() => setActivities([]));
    getNatureSpots(itinerary.trip.originLat, itinerary.trip.originLng).then(setNatureSpots).catch(() => setNatureSpots([]));
  }, [itinerary.trip.startDate, itinerary.trip.endDate, itinerary.trip.originLat, itinerary.trip.originLng]);
  const counts = { FESTIVAL: events.filter((event) => (event.eventType ?? "FESTIVAL") === "FESTIVAL").length, NIGHT_MARKET: events.filter((event) => event.eventType === "NIGHT_MARKET").length, TRADITIONAL_MARKET: events.filter((event) => event.eventType === "TRADITIONAL_MARKET").length, ACTIVITY: activities.length, NATURE: natureSpots.length, SOUVENIR: shops.length };

  const renderEvents = (type: Exclude<DiscoveryTab, "SOUVENIR" | "ACTIVITY" | "NATURE">) => {
  ...
  const renderSpots = (type: "ACTIVITY" | "NATURE") => {
    const spots = type === "ACTIVITY" ? activities : natureSpots;
    const emptyText = type === "ACTIVITY" ? "출발지 주변에서 확인된 액티비티가 없습니다." : "출발지 주변에서 확인된 산책·자연 스팟이 없습니다.";
    const typeLabel = type === "ACTIVITY" ? "LOCAL ACTIVITY" : "NATURE & WALK";
    return <div className="discovery-content" role="region" aria-label={`${labels[type].title} 목록`}>
      {spots.length === 0 && <p className="discovery-empty">{emptyText}</p>}
      {spots.map((spot) => <article className="discovery-card" key={spot.id}>
        ...
        <div className="discovery-card-copy">
          <span className="discovery-type">{typeLabel}</span>
          <h3>{spot.nameKo}</h3>
  ...
  return <section className="experience-panel discovery-panel" aria-label="축제 야시장 전통시장 액티비티 산책 자연 기념품샵">
    {(Object.keys(labels) as DiscoveryTab[]).map((type) => <section className={`discovery-bar-section ${active === type ? "open" : ""}`} key={type}>
      ...
      {active === type && <div id={`discovery-${type}`}>{type === "SOUVENIR" ? renderShops() : type === "ACTIVITY" || type === "NATURE" ? renderSpots(type) : renderEvents(type)}</div>}
    </section>)}
