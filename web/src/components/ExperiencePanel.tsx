...
import { addFestival, getActivities, getEvents, getSouvenirShops } from "../api/client";
import type { ActivitySpot, Festival, ItineraryOutput, SouvenirShop } from "../types";

type DiscoveryTab = "FESTIVAL" | "NIGHT_MARKET" | "TRADITIONAL_MARKET" | "ACTIVITY" | "SOUVENIR";
const labels: Record<DiscoveryTab, { title: string; description: string; index: string }> = {
  ...
  ACTIVITY: { title: "액티비티", description: "부산에서 즐길 수 있는 체험과 액티비티", index: "04" },
  SOUVENIR: { title: "기념품샵", description: "지역의 기억을 담아갈 가까운 상점", index: "05" },
};
...
export function ExperiencePanel({ itinerary, canEdit = true }: { itinerary: ItineraryOutput; canEdit?: boolean }) {
  ...
  const counts = { FESTIVAL: events.filter((event) => (event.eventType ?? "FESTIVAL") === "FESTIVAL").length, NIGHT_MARKET: events.filter((event) => event.eventType === "NIGHT_MARKET").length, TRADITIONAL_MARKET: events.filter((event) => event.eventType === "TRADITIONAL_MARKET").length, ACTIVITY: activities.length, SOUVENIR: shops.length };

  const renderEvents = (type: Exclude<DiscoveryTab, "SOUVENIR" | "ACTIVITY">) => {
  ...
  const renderActivities = () => <div className="discovery-content" role="region" aria-label="액티비티 목록">
    {activities.length === 0 && <p className="discovery-empty">출발지 주변에서 확인된 액티비티가 없습니다.</p>}
    {activities.map((spot) => <article className="discovery-card" key={spot.id}>
      {spot.imageUrl ? <img src={spot.imageUrl} alt={`${spot.nameKo} 이미지`} /> : <div className="discovery-art" aria-hidden="true"><span>ACTIVITY</span></div>}
      <div className="discovery-card-copy">
        <span className="discovery-type">LOCAL ACTIVITY</span>
        <h3>{spot.nameKo}</h3>
  ...
  return <section className="experience-panel discovery-panel" aria-label="축제 야시장 전통시장 액티비티 기념품샵">
    {(Object.keys(labels) as DiscoveryTab[]).map((type) => <section className={`discovery-bar-section ${active === type ? "open" : ""}`} key={type}>
      ...
      {active === type && <div id={`discovery-${type}`}>{type === "SOUVENIR" ? renderShops() : type === "ACTIVITY" ? renderActivities() : renderEvents(type)}</div>}
    </section>)}
