...
export function ShareBar({ itineraryId, tripId, language = "KO" }: { itineraryId: string; tripId: string; language?: "KO" | "EN" }) {
  ...
  return <section className="share-bar prep-collab-card" aria-label="일정 공유와 공동 편집">
    <header className="prep-card-heading"><div><span>{en ? "TOGETHER" : "동행"}</span><strong>{en ? "Plan together" : "동행자와 함께 계획하기"}</strong><small>{en ? "Invite companions with the right permission." : "편집 또는 열람 권한으로 동행자를 초대하세요."}</small></div><b>{en ? `${members} joined` : `${members}명 참여`}</b></header>
