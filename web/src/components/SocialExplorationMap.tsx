...
export function SocialExplorationMap({ tripId, itinerary }: Props) {
  ...
  const [activePanel, setActivePanel] = useState<"progress" | "layers" | "missions" | "surprise" | null>(null);
  ...
  const openPin = (pin: SocialMapPin) => { setActivePanel(null); setSelected(pin); if (activity.read === 0) recordActivity("read"); };
  ...
  useEffect(() => {
    ...
    loadKakaoSdk(key).then(() => {
      ...
      const paintFog = () => {
        ...
        const veil = document.createElementNS(ns, "rect"); veil.setAttribute("width", "100%"); veil.setAttribute("height", "100%"); veil.setAttribute("fill", "#dce9ee"); veil.setAttribute("fill-opacity", cleared ? ".36" : ".46"); veil.setAttribute("mask", "url(#sea-fog-mask)"); svg.appendChild(veil);
      };
      ...
      allItems.forEach((item) => {
        ...
        content.onclick = () => { setActivePanel(null); setCheckingPlaceId(item.placeId); };
  ...
  return <div className="social-map-shell map-first-shell">
    <div className="social-map-stage map-first-stage" ref={stageNode}>
      ...
      <div className="map-overlay-top">
        ...
        <button type="button" className={`map-layer-trigger ${activePanel === "layers" ? "active" : ""}`} aria-expanded={activePanel === "layers"} onClick={() => setActivePanel((value) => value === "layers" ? null : "layers")}><span>지도 옵션</span><b>{layers.length}개 레이어 · {scope === "me" ? "내 해무" : "동행 해무"}</b></button>
      </div>
      ...
      {activePanel === "layers" && <section className="map-option-panel layers-panel" aria-label="지도 표시 옵션"><button className="map-panel-close" type="button" onClick={() => setActivePanel(null)}>닫기</button><span>MAP OPTIONS</span><h2>지도에 무엇을 표시할까요?</h2><div className="layer-option-list" role="group" aria-label="지도 핀 레이어">{(Object.keys(LAYER_LABELS) as Array<keyof typeof LAYER_LABELS>).map((layer) => <button type="button" key={layer} className={layers.includes(layer) ? "active" : ""} aria-pressed={layers.includes(layer)} onClick={() => setLayers((values) => values.includes(layer) ? values.filter((value) => value !== layer) : [...values, layer])}><i style={{ background: LAYER_COLORS[layer] }} /><span>{LAYER_LABELS[layer]}</span><b>{layers.includes(layer) ? "표시 중" : "숨김"}</b></button>)}</div><button type="button" className="fog-scope-toggle" onClick={() => setScope((value) => value === "me" ? "party" : "me")}><span>해무 범위</span><b>{scope === "me" ? "내가 걸은 곳" : "동행이 함께 걸은 곳"}</b></button></section>}
      ...
      {activePanel === "surprise" && <section className="map-option-panel surprise-panel" aria-live="polite"><button className="map-panel-close" type="button" onClick={() => setActivePanel(null)}>닫기</button><span>SURPRISE TRAIL</span>{surpriseItem ? <><h2>DAY {surpriseItem.dayIndex} · {surpriseItem.nameKo}</h2><p>{surpriseItem.address}</p></> : <><h2>오늘의 탐험 장소</h2><p>실제 일정 장소 중 하나를 가볍게 골라드려요.</p></>}<div>{surpriseItem && <button type="button" className="secondary-btn" onClick={() => { setActivePanel(null); setCheckingPlaceId(surpriseItem.placeId); }}>이곳에서 시작</button>}<button type="button" className="surprise-pick-btn" onClick={pickSurprise}>{surpriseItem ? "다시 뽑기" : "장소 뽑기"}</button></div></section>}

      {cleared === 0 && firstItem && activePanel === null && !checkingPlaceId && !notice && <section className="map-first-onboarding"><span>첫 발자국</span><strong>일정 핀을 눌러 해무를 걷어보세요</strong><button type="button" onClick={() => setCheckingPlaceId(firstItem.placeId)}>첫 장소에서 시작</button></section>}
      {!selectedItem && !selected && !storyPlaceId && <div className="map-overlay-actions" aria-label="발자국 지도 옵션"><button type="button" className={activePanel === "missions" ? "active" : ""} onClick={() => setActivePanel((value) => value === "missions" ? null : "missions")}>미션 <b>{missions.filter((mission) => mission.done).length}/{missions.length}</b></button><button type="button" onClick={() => setShowDex(true)}>부산 도감 <b>{fog?.dex.unlockedCount ?? 0}/{fog?.dex.totalCount ?? 12}</b></button><button type="button" className={activePanel === "surprise" ? "active" : ""} onClick={() => setActivePanel((value) => value === "surprise" ? null : "surprise")}>오늘의 장소</button></div>}
    ...
    {notice && !selectedItem && <div className="social-map-notice" role="status"><span>{notice}</span><button type="button" onClick={() => setNotice("")}>닫기</button></div>}
    {selectedItem && <div className="social-map-sheet visit-sheet"><button className="sheet-close" onClick={() => { setCheckingPlaceId(null); setNotice(""); }}>닫기</button><span>방문 인증 · DAY {selectedItem.dayIndex}</span><h2>{selectedItem.nameKo}</h2><p>현재 위치는 인증에만 사용하고 약 500m 격자로만 남깁니다.</p>{visitStarts()[selectedItem.placeId] && <div className="visit-stay-state"><b>체류 확인 중</b><span>장소에서 10분 이상 머문 뒤 인증해 주세요.</span></div>}<button className="primary-btn" type="button" onClick={() => void handleVisit()}>{visitStarts()[selectedItem.placeId] ? "방문 인증하기" : "이 장소에서 방문 시작"}</button></div>}
