...
export const ROUTE_TREE: RouteNode[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { id: "trip.map", path: "map", expoPath: "app/trips/[tripId]/map.tsx", titleKo: "발자국", titleEn: "Footprints", purpose: "방문 인증으로 부산 해무를 걷고 탐험 미션·도감·여행자 메모를 즐기는 독립 핵심 화면이다.", platform: "BOTH", access: "TRIP_VIEWER", apis: ["GET /api/map/fog", "GET /api/map/pins", "POST /api/places/:placeId/visits/verify"], owners: ["FE", "BE"], milestone: "M1_WEB", status: "DONE" },
      { id: "trip.discover", path: "discover", expoPath: "app/trips/[tripId]/discover/index.tsx", titleKo: "여행 준비 · 부산 로컬", titleEn: "Prepare · Local", purpose: "여행 준비 탭의 하위 화면에서 여행 기간과 겹치는 축제·야시장·전통시장·기념품샵을 확인한다.", platform: "BOTH", access: "TRIP_VIEWER", apis: ["GET /api/festivals", "GET /api/events", "GET /api/shops/souvenir", "POST /api/itineraries/:id/festivals/:placeId"], owners: ["FE", "BE"], milestone: "M1_WEB", status: "DONE" },
