import type { ActivitySpot, BookingOption, CourseCategory, CreateTripRequest, EmbeddedRoute, Festival, FogMapState, ItineraryJob, ItineraryOutput, LocationSearchResult, PaceForecast, PlaceAlternative, PlaceImageMatch, PlaceRecord, ReplanResult, RhythmProfile, SharedItinerary, SocialMapPin, SouvenirShop, SponsoredPlacement, StoryComment, StoryRecord, TaxiCard, WeatherForecast } from "../types";
...
export async function getFogMap(tripId: string, scope: "me" | "party" = "me"): Promise<FogMapState> { return handle(await fetch(`/api/map/fog?scope=${scope}&tripId=${encodeURIComponent(tripId)}`, { headers: await authHeaders() })); }
export async function getSocialMapPins(tripId: string, bounds: { south: number; west: number; north: number; east: number }, layers: string[]): Promise<SocialMapPin[]> { const query = new URLSearchParams({ tripId, layers: layers.join(","), ...Object.fromEntries(Object.entries(bounds).map(([key, value]) => [key, String(value)])) }); return (await handle<{ pins: SocialMapPin[] }>(await fetch(`/api/map/pins?${query}`, { headers: await authHeaders() }))).pins; }
export async function verifyVisit(placeId: string, payload: { latitude: number; longitude: number; arrivedAt: string; departedAt: string }) { return handle<{ id: string; gridCell: string; fog: { clearedCells: number; totalCells: number; percent: number }; dexUnlocked: { tag: string; placeName: string } | null }>(await fetch(`/api/places/${placeId}/visits/verify`, { method: "POST", headers: { "Content-Type": "application/json", ...(await authHeaders()) }, body: JSON.stringify(payload) })); }
export async function getStoryComments(storyId: string): Promise<StoryComment[]> { return (await handle<{ comments: StoryComment[] }>(await fetch(`/api/stories/${storyId}/comments`, { headers: await authHeaders() }))).comments; }
export async function getStory(storyId: string): Promise<StoryRecord & { address: string; imageUrl: string | null; commentCount: number }> { return handle(await fetch(`/api/stories/${storyId}`, { headers: await authHeaders() })); }
export async function addStoryComment(storyId: string, body: string): Promise<StoryComment> { return handle(await fetch(`/api/stories/${storyId}/comments`, { method: "POST", headers: { "Content-Type": "application/json", ...(await authHeaders()) }, body: JSON.stringify({ body }) })); }
export async function joinParty(code: string) { return handle<{ tripId: string; role: string; joined: boolean }>(await fetch(`/api/party/${encodeURIComponent(code.trim().toUpperCase())}/join`, { method: "POST", headers: await authHeaders() })); }
