...
export async function updateStory(id: string, payload: { content: string; images: string[]; visibility: string }) { return handle(await fetch(`/api/stories/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json", ...(await authHeaders()) }, body: JSON.stringify(payload) })); }
