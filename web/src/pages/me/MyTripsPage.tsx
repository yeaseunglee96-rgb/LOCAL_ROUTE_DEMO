...
export function MyTripsPage() {
  ...
  return <main className="my-trips-page">
    ...
    {!loading && trips.length > 0 && <section className="my-trips-grid" aria-label="내 여행 목록">{trips.map((trip) => <article className="my-trip-card" key={trip.id}>
      <div className="my-trip-cover" style={trip.coverImage ? { backgroundImage: `url(${trip.coverImage})` } : undefined}>{!trip.coverImage && <span>LOCAL<br />ROUTE</span>}<em>{trip.storyCount > 0 ? `기록 ${trip.storyCount}개` : trip.itineraryId ? "일정 완성" : "작성 중"}</em></div>
