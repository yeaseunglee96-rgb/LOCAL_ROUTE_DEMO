...
**부제**: 현지인이 다시 가는 곳으로, 구글맵이 못 찾는 길도 걱정 없이 — 외국인을 위한 실행 가능한 로컬 여행 코스·내비게이션 플랫폼
...
LOCAL ROUTE(가칭)는 **한국을 여행하는 외국인**이 여행 조건(기간·예산·이동수단·취향·외국인 이용 조건)을 입력하면, 현지인이 실제로 검증한 장소 데이터와 운영시간·이동시간·예산 제약을 반영하여 **바로 실행 가능한 일 단위 여행 일정을 자동 생성**하는 여행 코스 최적화 서비스입니다.

기존 AI 여행 추천 서비스 다수는 유명 관광지를 나열하거나 자연어로 "대략적인 코스"를 생성하는 수준에 머물러 있습니다. LOCAL ROUTE는 다음 세 가지 지점에서 이를 구조적으로 넘어서는 것을 목표로 합니다.

첫째, 추천은 LLM의 직관이 아니라 **검증 가능한 로컬 점수를 가진 장소 데이터 + 제약조건 최적화 알고리즘**이 만듭니다. LLM은 결과를 설명하고 번역하는 역할만 담당합니다. 둘째, "현지인 추천"이라는 표현이 마케팅 문구로 그치지 않도록 GPS 방문 인증, 반복 방문, 리뷰 축적 등 **행동 기반의 로컬 인증 체계**를 둡니다. 셋째, 외국인 여행객을 부가 기능이 아니라 **일정 최적화의 1급 제약조건**으로 다룹니다. **[설계 제안]** 특히 한국은 지도 데이터 해외 반출 제한으로 구글맵의 도보·대중교통 길찾기가 정상 동작하지 않는 것으로 널리 알려져 있어, 외국인이 스스로 길을 찾지 못하는 문제가 구조적으로 존재합니다. LOCAL ROUTE는 이미 연동된 카카오모빌리티 경로 API를 기반으로 외국인이 그대로 따라갈 수 있는 내비게이션을 일정에 결합해 이 문제를 정면으로 다룹니다.

MVP는 부산, 한국어·영어, 외국인 실행 내비게이션(도보·대중교통·택시 턴바이턴 포함), 자차·도보 이동을 중심으로 6명 팀이 6주 안에 만들 수 있는 범위로 제한합니다. 여기에 **지역축제 안내, 기념품샵 지도, 날씨 안내, 동행자 공동 편집, 소셜(스토리·팔로우)** 까지 포함해 여행 전·중·후 경험을 잇습니다. 부산 외 지역으로의 전국 확장, 실시간 예약, 다국어 확대는 후속 단계로 분리합니다.
...
**[가정 표시]** 팀 인원 6명, 개발 기간 6주라는 조건은 사용자가 명시적으로 지정한 기본 가정입니다. 27장(6주 개발계획) 서두에서 이 가정이 본 기획서의 전체 범위(추천 엔진, 제약조건 최적화기, Kafka/Flink 실시간 파이프라인, 다국어, 외국인 내비게이션, 22개 화면, 24개 테이블 ERD, 로컬 인증 체계) 대비 왜 타이트한지, 어느 부분을 시연용으로 축소해야 하는지를 별도로 표시합니다.
...
원안: "한국을 여행하는 외국인이 현지인이 검증한 장소 및 실행 가능한 길찾기 정보를 바탕으로 자신의 여행기간, 예산, 이동수단과 취향에 맞는 실행 가능한 여행 코스를 자동 생성하는 서비스"

**[설계 제안] 개선된 정의**:

> "LOCAL ROUTE는 현지인이 실제로 반복 방문한 장소를 검증된 데이터로 확보하고, 여행자의 기간·예산·이동수단·취향 조건을 제약조건 최적화 알고리즘에 입력해 시간표 단위로 실행 가능한 여행 일정을 자동 생성하며, 구글맵이 놓치는 한국의 도보·대중교통 길찾기까지 그대로 따라갈 수 있는 내비게이션으로 이어주는, 한국 방문 외국인을 위한 로컬 여행 설계 서비스입니다."

### 2.3 핵심 메시지

> "관광객에게 유명한 곳이 아니라, 현지인이 다시 가는 곳으로.
> 구글맵이 못 찾는 길도, 외국인도 걱정 없이 떠나는 맞춤형 로컬 여행."

이 메시지는 서비스의 두 축(① 로컬 검증, ② 외국인을 위한 실행 가능한 내비게이션)을 그대로 반영하며, 이후 모든 기능·화면·수익모델 설계의 기준선으로 사용합니다.
...
| 외국인 길찾기 실패 | 한국 정부의 지도 데이터 해외 반출 제한으로 구글맵이 한국에서 도보·대중교통 길찾기를 제대로 제공하지 못해, 외국인이 현지에서 길을 잃는 사례가 널리 알려져 있음 [가설·추정] | 카카오모빌리티 경로 API 기반 턴바이턴 내비게이션을 일정에 결합, 택시 기사에게 보여줄 한글 목적지 카드 제공 |
...
| 서비스 유형 | 대표 성격 | 구조적 한계 |
| --- | --- | --- |
| 지도/내비게이션형 (카카오맵, 네이버지도, 구글맵 등) | 장소 검색과 길찾기는 강력하나 "일정"을 만들어주지 않음. 특히 구글맵은 한국의 지도 데이터 반출 제한으로 도보·대중교통 길찾기 자체가 부실함 [가설·추정] | 여행기간·예산 전체를 고려한 시간표 최적화 기능 부재, 외국인이 가장 많이 쓰는 구글맵이 한국에서 제 역할을 못 함 |
| 콘텐츠 큐레이션형 (여행 블로그, SNS 맛집 리스트) | 풍부한 콘텐츠, 감성적 소개 | 정보 최신성·신뢰성 검증 체계 부재, 협찬 구분 불명확, 개인화 없음 |
| AI 챗봇형 여행 플래너 | 자연어로 빠르게 코스 초안 제공 | LLM이 운영시간·이동시간·예산을 정확히 계산하지 못해 실행 불가능한 일정을 만드는 경우가 많음, 외국인 조건 반영 미흡 |
| 예약 플랫폼형 (OTA, 액티비티 예약) | 예약 전환에 최적화 | "일정 설계"보다 "개별 상품 판매"에 집중, 코스 간 이동시간·동선 고려 없음 |
| 번역·여행 편의 앱 (Papago 등) | 실시간 번역, 개별 상황 대응 | 여행 일정 자체를 설계해주지 않고, 길찾기·장소 신뢰도 문제는 그대로 남음 |

LOCAL ROUTE는 이 다섯 카테고리의 교집합 — **지도 시각화 + 로컬 검증 콘텐츠 + 제약조건 기반 일정 자동화 + 외국인 실행 내비게이션 + 예약 연결** — 을 하나의 파이프라인으로 통합하는 것을 차별점으로 삼습니다.
...
| 외국인 여행객 | 한국 방문 경험이 적거나 처음이며 언어·결제·교통(특히 구글맵 길찾기 부실)에 불편을 느끼는 여행객. 영어권 우선, 서비스의 핵심 타깃 | MVP 포함(한국어·영어) |
...
### 페르소나 2. 외국인 여행객 — Emily Carter(34세, 미국, 첫 방한)

- 목표: 2박 3일 부산 여행, 영어 메뉴가 있는 식당 선호, 환승 최소화
- 좌절: 구글맵으로 도보·대중교통 경로를 검색해도 안내가 부정확하거나 아예 나오지 않아 현지에서 몇 번이나 길을 잃음. "믿을 만한 맛집"을 영어로 확인하기도 어려움
- 필요 기능: 다국어 안내, 턴바이턴 도보·대중교통 내비게이션, 해외 카드 결제 가능 장소 필터, 택시 기사에게 보여줄 한글 목적지 카드
- 성공 시나리오: 구글맵 없이도 앱 하나로 부산 시내를 헤매지 않고 이동할 수 있는, 실행 가능한 3일 일정을 확보

### 페르소나 3. 초행 배낭여행객 — Daniel Kim(27세, 캐나다, 교포 3세, 첫 단독 방한)

- 목표: 4박 5일 자유여행으로 서울·부산을 도보와 대중교통으로 이동하며 현지인처럼 다니고 싶음
- 좌절: 첫날 구글맵만 믿고 도보 이동을 시작했다가 안내가 중간에 끊기거나 실제 도로와 달라 30분 넘게 헤맴. 지하철 환승 안내가 부정확해 반대 방향 열차를 탄 적도 있음. 한국어를 전혀 읽지 못해 택시 기사에게 목적지를 설명하지 못함
- 필요 기능: 정확한 도보·대중교통 턴바이턴 안내, 환승 난이도 요약, 택시 기사에게 보여줄 한글 목적지 카드, 도보 난이도(경사·계단) 사전 안내
- 성공 시나리오: 구글맵 없이 앱의 경로 안내만으로 5일 내내 목적지에 정확히 도착하고, 택시 카드 기능으로 언어 장벽 없이 이동

### 페르소나 4. 로컬 큐레이터(공급자) — 이하늘(41세, 부산 영도 카페 사장 겸 로컬 인플루언서)

- 목표: 자신이 실제로 다니는 동네 상권을 코스로 만들어 소개하고, 자신의 가게 정보를 정확히 관리하고 싶음
- 좌절: 지도 서비스에 등록된 영업시간·정보가 실제와 달라 외국인 손님과 마찰이 생김
- 필요 기능: 업체 관리자 화면(영업시간·메뉴 직접 수정), 로컬 코스 작성 및 판매, 노출·클릭 통계
- 성공 시나리오: 본인 코스가 외국인 여행객에게 실제로 사용되고, 코스 판매 수익과 가게 방문 전환을 함께 확인
...
맛집·카페·자연·사진·역사·문화·체험·쇼핑·액티비티·휴식·야경·아이 동반·실내 활동·숨은 로컬 명소·대표 관광지·도보 여행 — 총 15개 취향 태그. 각 태그는 장소 데이터의 카테고리·태그와 매칭되어 추천 점수의 "취향 일치도" 항목에 반영됩니다(15장 참고).
...
**[설계 제안]** 13장 외국인 내비게이션 시스템의 입력값입니다. 실시간 API 응답을 그대로 사용하므로 별도 정책 테이블 없이 요청 시점에 계산합니다.

| 입력 항목 | 타입 | 활용처 |
| --- | --- | --- |
| 도보 선호도 | 여유롭게/보통/최소화 | 경로 후보 중 도보 구간 비중 조정 |
| 대중교통·택시 우선순위 | 대중교통 우선/택시 우선/혼합 | 경로 옵션 정렬 기준 |
| 계단·경사 회피 여부 | Boolean | 도보 난이도 필터 |
| 캐리어·짐 동반 여부 | Boolean | 계단 많은 도보 구간 회피 가중 |
...
| 외국인 여행객 | 언어·결제·알레르기·환승 조건을 반영한, 현지인이 검증한 코스를 언어 장벽 없이 이용. 구글맵이 못 해주는 정확한 도보·대중교통 턴바이턴 안내로 현지에서 헤매지 않음 |
...
| 외국인 내비게이션 | 지도 앱을 별도로 켜서 알아서 찾아가야 함(한국에서는 구글맵 길찾기 자체가 부실) | 일정에 결합된 턴바이턴 내비게이션(도보·대중교통 영어 안내, 환승 난이도, 택시용 한글 카드) |
...
| 추천 이동수단 | 자차/도보/대중교통/택시(MVP, 13장 내비게이션) |
| 예상 교통비 | 주유비 환산 또는 대중교통·택시 요금(카카오모빌리티 API 기준) |
| 예상 이용금액 | 장소 가격대 기반 1인 예상 지출 |
| 추천 이유 | 로컬 점수·재방문율·거리 등 근거 문장(15장 참고) |
| 예약 필요 여부 | 장소 데이터의 예약 필요 여부 필드 반영 |
| 환승 난이도 요약 | 도보 구간·환승 횟수·계단 여부를 반영한 이동 난이도 표시 |
...
| 초행 외국인 안심 코스 | 첫 방문·환승에 자신 없는 외국인 여행객 | 환승 횟수 최소화, 도보 난이도 낮은 경로 우선, 영어 응대·해외카드 결제 가능 장소 우선, 각 구간에 턴바이턴 내비게이션 자동 포함 | 도보 선호도 "최소화" 설정 시 대중교통·택시 비중 상향 |

**[설계 제안] 추천 비율 슬라이더**: 사용자는 대표 관광지/현지인 추천/이동 편의 비율을 (기본값 30/50/20) 조정할 수 있으며, 이 비율은 15장의 추천 점수 계산식에서 카테고리별 후보 수 배분 가중치로 사용됩니다. 세 모드는 이 슬라이더의 프리셋(관광 필수=70/20/10, 현지인=10/80/10, 초행 외국인 안심=20/20/60 등, **[설계 제안]** 수치는 파일럿 데이터로 튜닝 필요)입니다.
...
| 경로 안내 오류 신고 | 사용자 신고(`nav-events`) | 해당 구간 경로를 카카오맵 딥링크 폴백으로 전환 + 운영팀 검토 큐 등록 |
...
외국인 사용자에게는 축제 장소까지의 경로도 13장 내비게이션 로직을 그대로 적용해, 환승 난이도와 택시 목적지 카드를 함께 제공합니다.
...
- 복제 시 사용자의 여행 기간·예산·이동수단 조건에 맞춰 **자동 재최적화**됩니다(21.14 로컬 코스 적용과 동일 흐름).
...
- 폭염·한파·강설 시에는 도보 구간이 많은 경로에 **대중교통·택시 대체 경로 안내**를 함께 제공합니다(13장 내비게이션 연동).
...
> 입력값(구조화 데이터): `{local_recommenders: 38, area: "영도", revisit_rate_3m: 0.62, next_place_distance_min: 8, transport: "car", transfer_count: 1}`
> LLM 출력(자연어 설명): "영도 로컬 38명이 추천했고, 최근 3개월간 재방문율이 높습니다. 다음 장소까지 환승 1회, 차량으로는 8분 거리입니다."
...
**[가설·추정]** 한국 정부는 안보상의 이유로 정밀 지도 데이터의 해외 반출을 제한하고 있으며, 이 때문에 구글맵은 한국에서 도보·대중교통 길찾기(턴바이턴 내비게이션)를 사실상 제공하지 못하는 것으로 널리 알려져 있습니다. 외국인 여행객 다수가 평소 쓰던 구글맵을 그대로 켰다가 경로 안내가 나오지 않거나 부정확해 현지에서 길을 잃습니다. 반면 카카오맵·카카오모빌리티는 국내 정밀 경로 데이터를 보유하고 있고, 서비스에는 이미 이 API가 연동되어 있습니다(`services/kakao.ts`, `GET /api/routes/directions`). 이 장은 이 연동을 "지도에 선 하나 긋는 것"에서 "외국인이 그대로 따라 걸을 수 있는 내비게이션"으로 격상시키는 방식을 정의합니다.

내비게이션은 "부가 화면"이 아니라 **일정의 모든 이동 구간에 기본으로 결합되는 1급 기능**으로 다룹니다. 일정 결과의 모든 장소-장소 이동 구간은 경로 요약(예상 소요시간·환승 횟수·도보 난이도)을 항상 동반하며, 사용자가 탭하면 단계별 턴바이턴 화면으로 진입합니다.

### 13.2 경로 데이터 신뢰도 관리

반려동물 정책 같은 크라우드소싱 데이터와 달리, 경로 데이터는 카카오모빌리티 API가 **요청 시점마다 실시간으로 반환**하는 값입니다. 따라서 "확인일·확인자 수" 같은 신뢰도 관리 대신 **API 응답 자체의 지연·정확도를 운영 지표로 관리**합니다.

| 관리 항목 | 처리 |
| --- | --- |
| API 응답 지연 | 임계치(예: 2초) 초과 시 캐시된 최근 경로로 폴백하고 "예상 경로(참고용)" 배지 표시 |
| API 오류·쿼터 초과 | 카카오맵 딥링크로 폴백(외부 앱 연결), 사용자에게 실시간 안내 불가 사유 명시 |
| 경로 결과 없음(도보 불가 구간 등) | "도보 경로 없음 — 대중교통·택시 이용 권장"으로 명시, 임의로 추정 경로를 만들지 않음 |
| 사용자 피드백 | "경로가 실제와 다름" 신고 버튼 → 운영팀 검토 큐(카카오 API 응답 자체의 오차이므로 정책처럼 자동 하향 조정할 대상은 없음) |

### 13.3 턴바이턴 내비게이션 로직

1. 일정의 각 이동 구간에 대해 사용자의 이동수단 선호(자차/도보/대중교통/택시)에 따라 카카오모빌리티 경로 API를 호출
2. 도보·대중교통 경로는 **단계별 영어 안내 텍스트**(예: "200m 직진 후 우회전", "2호선으로 환승, 3번 출구 방향")로 변환해 순서대로 표시
3. 환승이 있는 구간은 환승 횟수·환승역명·환승 소요시간을 요약해 "환승 난이도"(쉬움/보통/어려움)로 표시
4. 도보 경로에 계단·경사 구간이 포함되면 "도보 난이도" 배지를 추가(캐리어·유모차 동반 시 회피 경로 우선 탐색)
5. 사용자가 "택시 이용"을 선택하면 목적지 주소·장소명을 큰 글씨의 **한글 목적지 카드**로 생성해 기사에게 바로 보여줄 수 있게 함(14.2장 한글 문장 템플릿과 동일 원칙)
6. 도보·대중교통 경로가 아예 없는 구간은 자동으로 택시 옵션을 기본 대안으로 제시

### 13.4 내비게이션 데이터 항목

예상 소요시간, 이동수단별 경로 옵션(도보/대중교통/택시/자차), 환승 횟수, 환승역명, 도보 난이도(계단·경사 유무), 택시 예상요금(카카오모빌리티 응답 기준), 한글 목적지 카드 텍스트, 단계별 영어 안내 텍스트 배열 — 이 데이터는 **요청 시점에 카카오모빌리티 API 응답으로 계산**되며, 반려동물 정책처럼 별도로 저장·관리하는 DB 테이블을 두지 않습니다(20장 ERD에 관련 테이블을 추가하지 않는 이유).
...
**[설계 원칙]** 외국인에게 중요한 정보(주소, 영업시간, 가격, 경로 안내, 알레르기 유발 성분)는 LLM이 매번 새로 생성하는 자유 문장보다 **사전에 구조화된 데이터 + 아이콘**을 우선 노출합니다. LLM은 구조화된 데이터를 문맥에 맞게 요약·설명하는 보조 역할만 수행하며, 원본 구조화 데이터는 항상 함께 제공되어 자유 문장의 오류를 사용자가 교차 확인할 수 있게 합니다.
...
13장 내비게이션 시스템이 이 절의 실제 구현체입니다. "환승 허용 횟수" 입력값은 카카오모빌리티 대중교통 경로 API 호출 시 경로 후보 필터로 직접 사용되며, MVP부터 도보·대중교통 턴바이턴 안내를 제공합니다. 외부 앱으로의 딥링크는 API 오류·쿼터 초과 시의 폴백(13.2장)으로만 사용합니다.
...
| 이동 편의성(환승·도보 난이도) | 15% | 13장 내비게이션 API의 환승 횟수·도보 난이도 응답 기반 점수 |
| 예산 적합도 | 10% | 장소 가격대와 남은 일별 예산의 근접도 |
| 외국인 이용 편의성 | 10% | 영어 메뉴·응대·해외카드·온라인예약 boolean 합산 정규화 |
| 정보 최신성 | 5% | 마지막 확인일 기반 감쇠 함수 |

**정규화 규칙**: 이동 편의성 점수는 모든 사용자에게 동일하게 적용되는 항목이므로 별도 재정규화가 필요하지 않습니다. 단, 자차 이동을 선택한 사용자는 "이동 편의성" 항목을 제외하고 나머지 6개 항목의 가중치를 합이 100%가 되도록 비례 재배분합니다(예: 25/20/15/10/10/5 → 각 항목을 0.85로 나누어 재정규화 ≈ 29.4/23.5/17.6/11.8/11.8/5.9%).
...
모든 추천 결과는 사용자에게 "왜 이 장소인가"를 근거값(로컬 추천 수, 재방문율, 이동시간, 환승 난이도 등)과 함께 제공합니다. LLM은 이 근거값을 문장으로 변환할 뿐, 근거값 자체를 만들어내지 않습니다(28조 원칙 1·4).
...
- 외국인 환승 난이도(대중교통 경로 선택 시 가중치, 13장 내비게이션 API 연동)
- 도보 난이도(계단·경사 회피 여부, 캐리어·유모차 동반 시 가중)
- 마지막 날 귀가시간(마지막 노드에서 출발지 복귀 시간 hard constraint)

### 16.2 목적함수(예시)

> 방문 장소 추천 점수의 합을 최대화하면서, 이동시간·예산 초과·일정 과밀·운영시간 위반·환승 난이도를 최소화한다.
...
minimize  λ1·total_travel_time
        ...
        + λ5·transfer_difficulty_penalty
subject to
...
| 3. 장소 정보 보강 | 부족한 필드(외국어 지원 여부 등)를 자체 조사/제휴 데이터로 보강, 없으면 "정보 없음"으로 명시 | 임의 추정 금지 |
| 4. 필수 조건 필터링 | 제외 장소, 알레르기 위반, 운영일 불일치 등 hard constraint 제거 | |
...
| 카카오모빌리티 대중교통·도보 길찾기 API | **외국인 내비게이션(13장)** — 턴바이턴 도보·대중교통 경로, 환승 안내 | 단계별 경로, 환승역, 소요시간 | 백엔드 | `Authorization: KakaoAK {REST_KEY}` 헤더 | Redis 캐싱(16.5장) | **추가 확인 필요**(자동차 길찾기와 별도 쿼터 여부) | 카카오맵 딥링크로 폴백(13.2장) | 예 | 대량 사용 시 제휴 문의 필요 |
...
| 국토교통부 TAGO 교통정보 | 대중교통 노선·정류장 정보(전국 확장 시 보강용) | 버스·지하철 노선, 정류장 | 백엔드 | 공공데이터포털 인증키 | 배치 캐싱 | **추가 확인 필요** | 카카오모빌리티 API로 대체 | 아니오(향후) | — |
...
**공식 참고 문서**: [Kakao Developers 쿼터 안내](https://developers.kakao.com/docs/latest/ko/getting-started/quota) · [카카오모빌리티 길찾기 API 가이드](https://developers.kakaomobility.com/guide/navi-api/waypoints) · [카카오 데브톡 길찾기 요금 안내](https://devtalk.kakao.com/t/api/142931) · [공공데이터포털 국문 관광정보 서비스](https://www.data.go.kr/data/15101578/openapi.do) · [TAGO 국가대중교통정보센터](https://www.tago.go.kr/)

**[추가 확인 필요]** 공공데이터포털 API는 통상 "개발계정" 단계에서 낮은 일일 트래픽 한도(플랫폼 관행상 데이터셋별 상이)로 시작해 "운영계정" 승인 후 상향되는 구조이나, 본 조사에서 TourAPI·TAGO의 정확한 계정별 일일 한도 수치는 공식 문서에서 확정적으로 확인하지 못했습니다. 개발 착수 전 공공데이터포털에서 각 데이터셋의 "활용신청" 페이지의 트래픽 한도를 직접 확인해야 합니다. 카카오맵의 "첫 번째 앱만 무료 쿼터 적용" 정책도 팀의 앱 등록 상황에 따라 실제 적용 여부를 재확인해야 합니다. 카카오모빌리티 대중교통·도보 길찾기 API의 정확한 무료 쿼터·요율도 자동차 길찾기와 별도로 확인이 필요합니다.
...
여행 검색, 여행 일정 생성, 장소 노출, 장소 클릭, 장소 저장, 장소 제외, 장소 고정, 장소 순서 변경, 일정 확정, 일정 공유, 길찾기 시작, 턴바이턴 내비게이션 완료, 택시 카드 생성, 장소 방문 인증, 장소 재방문, 여행 완료, 추천 만족도, 예산 초과, 로컬 장소 추천, 로컬 코스 작성, 로컬 코스 사용, 경로 안내 오류 신고, 번역 수정, 광고 노출, 광고 클릭, 쿠폰 저장, 예약 시작, 예약 완료, 예약 취소 — 기본 27종.
...
| `place-policy-events` | 장소 정보 변경(저빈도, 신뢰성 중요) | `{event:"place_info_updated", place_id, field, old, new, source}` | `place_id` | 1년 | PlaceSvc(정보 캐시 갱신), 알림 서비스(향후) | `event_id` + 버전 번호(낙관적 락) | 없음 |
...
| `nav-events` | 턴바이턴 내비게이션 시작/완료, 택시 카드 생성, 경로 오류 신고 | `{event:"nav_route_reported_wrong", place_id, user_id, leg_id, note}` | `trip_id` | 1년 | Flink(내비게이션 이용률·API 오차 집계), 운영팀 검토 큐 | `event_id` | 있음(가명화) |
...
**결론(권장): Flink**. 급상승 장소 집계, Consumer Lag 최소화, 내비게이션 API 오류율의 저지연 반영, 광고 예산 실시간 차감처럼 **초 단위 지연시간이 중요한 요구사항**이 많고, 22장에서 심사위원에게 파티션·Consumer 확장에 따른 처리량 변화를 시연해야 하므로 네이티브 스트리밍 모델이 유리합니다. 다만 6주 MVP에서는 Flink 학습 곡선을 고려해 핵심 지표(급상승 장소, Consumer Lag, 내비게이션 이용률) 3~4개만 구현하고, 나머지는 배치로 대체하는 단계적 접근을 권장합니다(27장 참고).

Flink로 계산할 지표: 최근 급상승 장소, 지역별 인기 장소, 언어별 인기 장소, 현지인의 재방문 장소, 외국인 내비게이션 이용률·API 응답 오차 알림 트리거, 코스 완주율, 일정 변경률, 추천 후 제외율, 광고 전환율, 사용자별 최근 여행 취향(세션 윈도우), 지역별 로컬 점수(증분 갱신), 비정상 클릭·리뷰·방문 인증(간단한 룰 기반 이상탐지, 정교한 모델은 배치로 보강).
...
| 경로 안내 API 응답 지연시간 측정 | 카카오모빌리티 경로 API 호출 발행부터 턴바이턴 안내 화면 표시까지의 지연시간을 부하 상황에서 측정 |
...
**설계 메모**: `AdCampaign.business_id`가 참조할 `Business`(업체 계정) 테이블은 12장·22장에서 다루는 업체 관리자 화면의 기반이 되나, ERD 복잡도를 낮추기 위해 본 다이어그램에서는 생략하고 4주차 이후 업체 구독 기능 구현 시점에 추가합니다(27장 참고).
...
- 인증: 필요
- 요청 예시: `{"preference": {"tasteTags": ["local_food","cafe","sea_view"], "landmarkRatio":30,"localRatio":50,"navConvenienceRatio":20}, "navPreferenceId": "nav_11"}`
...
- 인증: 필요
- 응답 예시: 최대 5개 대체 후보(추천 점수순), 각 후보에 외국인 조건 충족 여부 포함
...
- 인증: 불필요(공개)
- 응답: Place + PlaceTranslation(lang) + 최근 리뷰 요약
- 오류코드: `404 PLACE_NOT_FOUND`
- 캐싱: CDN/Redis 캐시 TTL 10분

### 21.11 경로 안내(내비게이션)

- `GET /v1/routes/directions?from={lat,lng}&to={lat,lng}&mode={walk|transit|taxi|car}&lang=en`
- 인증: 불필요
- 응답 예시: `{"mode":"transit","durationMin":22,"transferCount":1,"walkDifficulty":"EASY","steps":[{"instruction":"Walk 200m north","distanceM":200},{"instruction":"Transfer to Line 2 at Seomyeon, exit 3","transferStation":"서면"}],"taxiCard":{"destinationKo":"부산광역시 영도구 ○○로 12","estimatedFareKrw":8500}}`
- 오류코드: `404 ROUTE_NOT_FOUND`(해당 이동수단으로 경로 없음 — "도보 경로 없음, 대중교통·택시 권장"으로 명시), `502 UPSTREAM_TIMEOUT`(카카오모빌리티 응답 지연 시 캐시 폴백, 13.2장 참고)
- 캐싱: Redis TTL 6시간(16.5장과 동일 정책)
...
- 인증: 불필요(공개)
- 응답 예시: `{"festivals":[{"placeId":"...","title":"부산불꽃축제","startDate":"2026-10-01","endDate":"2026-10-02","playTime":"19:00~21:00","transferDifficulty":null}]}`
- 오류코드: `400 INVALID_DATE_RANGE`
- 비고: `transferDifficulty: null`은 "정보 없음"이며 임의로 "쉬움"으로 해석하지 않습니다.
...
| 상단 고정바 | 예상 누적비용, 부분 재최적화 버튼, 모드 전환(관광 필수/현지인/초행 외국인 안심) |
...
| --- | --- | --- | --- | --- | --- |
| 1 | 온보딩 | 서비스 핵심 가치 전달 | 3장 슬라이드(로컬 검증/외국인 내비게이션/외국인 편의), 시작 버튼 | 스와이프, 건너뛰기 | 재방문 사용자는 자동 스킵 |
...
| 4 | 취향 선택 | 7.2 취향 태그 및 비율 선택 | 15개 태그 칩, 대표관광지/현지인/이동편의 비율 슬라이더(기본 30/50/20) | 다중 선택, 슬라이더 조정 | 취향 0개 선택 시 기본 균등 가중치로 안내 |
| 5 | 내비게이션 설정 | 7.4 이동 편의 정보 입력 | 도보 선호도, 대중교통/택시 우선순위 선택, 계단·경사 회피·캐리어 동반 토글 | 설정 저장(재사용 가능) | 도보 최소화 + 캐리어 동반 시 "택시 위주로 안내됨" 사전 안내 |
...
| 9 | 장소 상세 | 개별 장소 정보 확인 | 이미지, 설명, 운영시간, 가격대, 외국인 이용 편의 아이콘 | 저장, 제외, 리뷰 열람 | 정보 최신성 낮을 시 경고 배너 |
| 10 | 현지인 추천 근거 | 로컬 점수 신뢰 근거 제공 | 12.5장 구조화 근거 + 자연어 요약 | 근거 상세 펼치기 | 로컬 추천 데이터 부족 시 "축적 중" 안내 |
| 11 | 경로 상세 | 턴바이턴 내비게이션 확인 | 13.4 필드 전체(단계별 안내, 환승 정보, 택시 카드) | 택시 카드 확대 보기, 경로 오류 신고 | 경로 없음(ROUTE_NOT_FOUND) 시 대중교통·택시 대안 배너 |
...
| 20 | 업체 관리자 화면 | 12.3 업체 정보 직접 관리 | 영업시간·메뉴·쿠폰 편집 폼, 통계 대시보드 | 정보 수정 요청(검수 후 반영), 통계 열람 | 수정 요청이 검수 대기 중일 때 상태 표시 |
...
| 23 | 지역축제 목록·상세 | 11.5.1 기간 한정 축제 안내 | 여행 기간 내 축제 카드, 개최기간·장소·요금, 경로 요약 | 일정에 추가, 상세 열람 | 해당 기간 축제 없음 → 빈 상태 안내(억지 추천 금지) |
...
모든 장소 카드·일정 항목은 한글명/영문명, 한글 주소/영문(로마자) 주소, 택시 기사에게 보여줄 한글 문장 카드를 함께 제공합니다(14.2장). 언어 설정이 English인 경우 구조화된 아이콘(경로 난이도, 알레르기, 결제)을 텍스트보다 우선 배치합니다.
...
| 2순위 | 주요 차별화 기능(로컬 검증, 외국인 내비게이션, 광고 분리 등) |
| 3순위 | 사용성 향상 기능 |
| 4순위 | 사업화 이후 기능 |
| 5순위 | 장기 확장 기능 |
...
| FR-02 | 여행생성 | 취향 태그 선택 | 15개 취향 태그를 다중 선택하고 비율 슬라이더를 조정할 수 있다 | 0 | 태그 선택값이 API 요청 payload에 반영됨을 확인 |
| FR-03 | 내비게이션 | 내비게이션 설정 입력 | 도보 선호도·대중교통/택시 우선순위·계단 회피·캐리어 동반 여부를 입력하고 저장한다 | 0 | 저장된 설정이 다음 여행에 재사용됨을 확인 |
...
| FR-07 | 추천모드 | 3가지 추천 모드 제공 | 관광 필수/현지인/초행 외국인 안심 코스를 각각 생성한다 | 0 | 동일 조건에서 3개 모드의 장소 구성이 서로 다름을 확인(중복률 40% 이하) |
| FR-08 | 추천모드 | 추천 비율 슬라이더 | 대표관광지/현지인/이동편의 비율을 사용자가 조정 가능 | 2 | 슬라이더 값 변경 시 재생성된 일정의 카테고리 비중이 변경됨을 확인 |
...
| FR-17 | 내비게이션 | 턴바이턴 도보·대중교통 안내 | 이동 구간마다 단계별 영어 안내 텍스트와 환승 정보를 제공한다 | 0 | 대중교통 구간이 포함된 일정에서 환승역명·환승횟수가 표시됨을 확인 |
| FR-18 | 내비게이션 | 택시 목적지 카드 생성 | 택시 이용 시 목적지 주소·장소명을 큰 글씨 한글 카드로 생성한다 | 2 | 택시 모드 선택 시 한글 카드 화면이 노출됨을 확인 |
| FR-19 | 내비게이션 | 경로 API 응답 지연 폴백 | API 응답이 임계치를 초과하면 캐시된 경로 또는 카카오맵 딥링크로 폴백한다 | 1 | 강제 지연 주입 테스트에서 폴백 배지가 노출됨을 확인 |
| FR-20 | 내비게이션 | 도보 난이도 표시 | 계단·경사 구간이 포함된 도보 경로에 난이도 배지를 표시한다 | 2 | 계단 포함 경로 응답에 난이도 배지가 노출됨을 확인 |
...
| FR-34 | 광고 | 조건 불충족 광고 비노출 | 예산·운영시간·거리 조건에 맞지 않는 광고는 노출하지 않는다 | 1 | 조건 불충족 광고 캠페인이 결과에서 제외됨을 확인 |
...
| FR-38 | 업체관리 | 업체 정보 자가 관리 | 업체가 영업시간·메뉴·쿠폰을 수정 요청할 수 있다 | 4 | 수정 요청 후 검수 승인 시 Place 데이터에 반영됨을 확인 |
...
대상: 숙소, 렌터카, 체험, 입장권, 공항택시, 여행자 보험, eSIM·데이터.
...
| eSIM·데이터 | 여행 시작 전 | 외국인의 지도·번역·내비게이션 사용 전제 조건 |

**[설계 제안]** 화이트리스트 확장은 "이것이 없으면 일정을 실행할 수 없는가"라는 단일 기준으로만 판단합니다. 이 기준을 통과하는 후보로는 짐 보관·배송(체크인 전/후 이동 불가 문제 해결)이 있으며, 도입 시 별도 검토를 거칩니다.
...
**핵심 규칙(25장과 연동)**: 광고 후보도 운영시간·예산·거리 조건의 hard filter를 자연 추천과 동일하게 통과해야 하며, 조건 미충족 시 광고비 액수와 무관하게 노출하지 않습니다. 여기에 더해 **카테고리 화이트리스트를 통과하지 못하면 심사 단계에서 캠페인 등록 자체가 거부**됩니다.
...
업체 관리자 화면(22.2 #20)에서 다국어 장소 페이지, 영업시간, 메뉴·가격, 예약 링크, 노출·클릭·저장 통계, 일정 포함 횟수, 국가·언어별 이용 통계, 외국인 문의 번역을 관리합니다.

**[24.2장 정책 변경에 따른 재정의]** 광고 카테고리를 필수 서비스로 제한했으므로, 일반 업체용 상품에서 **광고 캠페인 기능은 제외**합니다. 업체 구독의 가치 제안은 "노출을 사는 것"이 아니라 **"내 가게 정보를 정확하게 유지하고, 여행자에게 어떻게 보이는지 아는 것"** 입니다. 이는 기획서 전체의 정보 정확성 원칙과도 일치하며, 영업시간·가격처럼 자주 바뀌는 정보를 업체가 직접 갱신하게 만드는 유인이기도 합니다.
...
| Global | 외국인 응대 특화 업체 | 59,000원 | 다국어 페이지, 경로 안내 노출 최적화, 외국인 문의 번역, 상세 통계 |
...
| 오프라인 저장 | 포함 | 포함 |
| 고급 내비게이션 옵션(택시 예상요금 상세, 우선순위 커스텀) | 포함 | 포함 |
...
개인 위치정보 원본은 판매하지 않고, 익명·집계 데이터로 다음을 제공합니다: 외국인이 많이 검색한 지역, 언어별 선호 관광지, 구글맵 길찾기 실패로 추정되는 이탈 구간, 코스 중도 포기 지역, 교통 불편으로 제외된 장소, 날씨에 따른 여행경로 변화, 차량·대중교통 여행객 차이, 환승 난이도가 높은 구간. 집계 단위는 k-익명성(예: 셀당 최소 사용자 수 이상일 때만 통계 공개)을 적용해 개인 재식별 가능성을 낮춥니다(18장 참고).
...
4. 광고 장소도 운영시간·예산·거리 조건의 hard filter를 자연 추천과 동일하게 통과해야 한다.
5. 유료 후기와 협찬 코스에는 경제적 이해관계를 명시적으로 표시한다.
6. 길찾기 화면과 턴바이턴 내비게이션 화면(13장)에는 광고를 노출하지 않는다.
7. 사용자의 알레르기 조건, 이동 편의 조건보다 광고를 우선하지 않는다(hard filter가 항상 먼저 적용).
8. 광고 카드에는 광고 추천 이유(왜 이 조건에 맞는지)와 광고주 정보를 확인할 수 있는 경로를 제공한다.
9. **광고는 여행 실행에 필수적인 카테고리(숙박·렌터카·택시·보험·공항이동·eSIM)로만 한정한다**(24.2장). 음식점·카페·기념품샵 등 취향 영역은 광고 상품으로 판매하지 않는다.
...
| KPI | 계산식 | 데이터 출처 |
| --- | --- | --- |
| 내비게이션 이용률 | 턴바이턴 내비게이션 시작 건수 / 이동 구간이 포함된 일정 항목 수 | `nav-events` |
| 턴바이턴 완주율 | 내비게이션 완료 건수 / 내비게이션 시작 건수 | `nav-events` |
| 경로 오류 신고율 | 경로 오류 신고 수 / 내비게이션 시작 건수 | `nav-events`(nav_route_reported_wrong) |
| 택시 카드 생성 건수 | 택시 목적지 카드 생성 이벤트 수 | `nav-events` |
...
| 구분 | 포함 |
| --- | --- |
| 반드시 구현(MVP 필수) | 여행 조건·취향·내비게이션 설정 입력, 규칙 기반 추천(15.1), OR-Tools 일정 생성(16장 핵심 파이프라인), 3가지 모드, 12개 필드 시간표, 카카오맵 시각화, 부분 재최적화(장소 고정/삭제/교체), 턴바이턴 내비게이션(13장, 도보·대중교통·택시), 한/영 다국어 UI, 로컬 점수 계산(규칙 기반), 방문 인증(GPS), Kafka 이벤트 발행(전 종류) + 핵심 지표 실시간 집계, 광고 자연추천 분리 구조, 지역축제 안내(11.5.1), 날씨 배지·우천 경보(11.8), **기념품샵 지도 레이어(11.5.2), 일정 공유 + 동행자 공동 편집(11.6), 소셜 — 스토리·팔로우·신고·검토 큐(11.7)** |
...
| 내비게이션 부가 요소(택시 예상요금 상세 등) 축소 | 1 인·일 | 핵심(턴바이턴 안내)은 유지 |
...
| 2주차 | 핵심 데이터·추천 | 백엔드 + 추천 | Place/Trip 스키마, 규칙 기반 추천 점수(15.1), 카카오모빌리티 경로 API 연동(13장 기초), **축제 데이터 수집·기간 필터** | 추천 점수 단위 테스트, 경로 API 응답 정확도, 기간 겹침 판정 검증 | 1주차 시드 | 로컬 점수용 초기 데이터 부족 | 추천 점수 + 축제 조회 API 데모 |
...
| 내비게이션 경로 검증 | 도보·대중교통·택시 모드별로 카카오모빌리티 응답이 단계별 안내·환승 정보로 정확히 변환되는지 검증 |
...
페르소나 2(Emily Carter)를 기반으로 한 3~5분 데모 시나리오: 처음 부산을 방문하는 영어권 외국인, 출발지 부산역, 2박 3일, 예산 30만원, 자차 없음, 취향은 바다·사진·현지인 맛집·카페, 영어 메뉴 필요, 환승 최소화, 도보 난이도 낮은 경로 선호.

### 29.2 장면 구성(총 11장면, 3~5분)

| # | 장면 | 보여줄 것 | 소요시간 목표 |
| --- | --- | --- | --- |
| 1 | 여행 조건 입력 | 언어를 English로 전환 → 부산역 출발, 2박3일, 30만원, 자차 없음, 도보 난이도 최소화 입력 | 30초 |
| 2 | 현지인 코스 vs 초행 외국인 안심 코스 비교 | 두 모드를 탭으로 전환하며 장소 구성·환승 난이도 차이를 시각적으로 대비 | 30초 |
| 3 | 지도에 날짜별 경로 표시 | 카카오맵에서 1일차/2일차/3일차 경로가 색상별로 구분됨을 확인 | 20초 |
| 4 | 로컬 추천 근거 확인 | "영도 로컬 38명이 추천, 재방문율 62%" 등 12.5장 예시 문장 실제 확인 | 20초 |
| 5 | 턴바이턴 내비게이션 확인 | 카페 이동 구간에서 단계별 영어 안내, 환승 난이도, 택시 목적지 카드 확인(구글맵과 나란히 비교해 구글맵은 경로가 안 나오는 것을 대비) | 25초 |
| 6 | 장소 삭제 + 부분 재최적화 | 2일차 카페 하나를 삭제 → 3초 이내 해당 구간만 재계산됨을 확인 | 30초 |
| 7 | 경로 API 응답 지연 폴백 시연 | 관리 콘솔(또는 스크립트)에서 카카오모빌리티 응답 지연을 강제 주입, 캐시 폴백·"예상 경로(참고용)" 배지 전환 확인 | 15초 |
| 8 | 경로 오류 신고 → 검토 큐 반영 | 사용자가 "경로가 실제와 다름"을 신고하면 운영팀 검토 큐(`nav-events`)에 적재됨을 확인 | 25초 |
...
| 11 | 숙소 예약 제휴 링크 연결 | 숙소 카드에서 예약 링크 클릭 → `booking-events`(STARTED) 기록 확인 | 20초 |
...
| 카카오모빌리티 API 요금·쿼터 초과 | 중 | 높음(핵심 기능 직결) | 16.5장 캐싱·군집화로 호출량 축소, 13.2장 지연·오류 시 폴백 체계, 임계치 도달 시 알림(NFR-18), 유료 전환 비용 사전 산정 |
...
| 턴바이턴 내비게이션 구현 범위 초과 | 높음 | 높음(핵심 차별점 직결) | MVP는 부산 지역으로 한정하고, 범위 초과 시 도보 난이도·택시 카드부터 덜어내고 기본 소요시간·환승 횟수 표시는 반드시 유지(27.2.2장과 동일 원칙) |
...
| 전국 대중교통 경로 추천 | TAGO 등 대중교통 API 통합, 환승 최적화 로직 개발 | 부산 MVP는 카카오모빌리티로 충분하나 전국 확장 시 지역별 데이터 커버리지 검증 필요 |
...
| 해외 관광지역 확장 | 국내 검증 완료, 현지 로컬 인증 체계 이식 가능성 검토 | 국가별 지도 데이터 반출 규정이 한국과 다르므로 내비게이션 설계(13장)를 그대로 이식할 수 없음 — 국가별 재검토 필요 |
...
| 2 | 웨이로컬(WayLocal) | "현지인의 길"을 의미, 외국인 사용자에게도 직관적 |
| 3 | 단골로(DANGOLRO) | "단골"(현지인이 반복 방문하는 곳) + "路(로/길)"의 한글 조어 |
| 4 | 로코스(LOCOURSE) | Local + Course의 축약, 코스 추천 서비스임을 직관적으로 전달 |
| 5 | 여기로컬(YeogiLocal) | "여기"(현재 위치)와 로컬을 결합, 지역 밀착 인상 |
| 6 | 트루로컬(TrueLocal) | 검증된 진짜 현지 정보라는 신뢰 메시지 강조 |
| 7 | 동네방네트립(DongneTrip) | "동네방네"라는 친숙한 한국어 표현과 여행(Trip)의 결합, 국내 사용자 친화적 |
| 8 | 고로케이트(GoLocate) | "가다(Go)"와 "위치를 찾다(Locate)"의 결합, 길찾기·내비게이션 정체성을 직접 드러냄 |
| 9 | 로컬내비(LocalNavi) | 로컬(Local)과 내비게이션(Navi)의 직접 결합, 외국인에게 기능이 즉시 전달됨 |
| 10 | 트루패스(TruePath) | 검증된 정보(True)와 실제 걸을 수 있는 경로(Path)를 결합, 영어권 사용자 대상 브랜딩에 적합 |

---

## 33. 최종 결론

LOCAL ROUTE는 "AI가 유명 관광지를 나열하는 서비스"와 스스로를 명확히 구분하는 세 가지 구조적 장치 위에 서 있습니다. 첫째, 일정의 최종 결정권은 LLM이 아니라 검증 가능한 데이터를 다루는 추천 엔진과 제약조건 최적화기에 있습니다. 둘째, "현지인 추천"은 행동 기반 인증과 공개된 점수 공식으로 뒷받침되며, 광고비는 이 점수 체계에 어떤 경로로도 영향을 주지 않습니다. 셋째, 한국을 여행하는 외국인은 부가 옵션이 아니라 서비스의 핵심 타깃이며, 구글맵이 한국에서 풀지 못하는 도보·대중교통 길찾기 문제를 일정 최적화와 결합된 턴바이턴 내비게이션으로 정면 해결합니다.
...
다음 단계로 권고하는 것은 다음과 같습니다. 부산 지역 장소 시드 데이터(최소 50~100곳)와 카카오모빌리티 경로 API 연동 검증을 1주차에 최대한 확보할 것, 로컬 점수·광고 분리 원칙이 코드 레벨에서 실제로 지켜지는지 검증하는 회귀 테스트를 초기부터 마련할 것, 그리고 6주차 데모는 프로덕션 배포가 아니라 핵심 가설(제약조건 기반 실행 가능한 일정, 검증된 로컬 추천, 구글맵을 대체하는 외국인 내비게이션, 광고와 추천의 분리, 실시간 분산처리)이 실제로 동작함을 증명하는 데 집중하는 것입니다.
