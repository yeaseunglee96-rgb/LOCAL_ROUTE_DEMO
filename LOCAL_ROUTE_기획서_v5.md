# LOCAL ROUTE 통합 서비스 기획서 v5.0 (상용 프로덕션 마스터)

**부제**: 현지인이 다시 가는 곳으로, 언어와 지도의 장벽 없이 — 외국인 현장 이용 완결형 스마트 로컬 여행 플랫폼

- **문서 버전**: v5.0 (프로덕션 마스터 기획서)
- **최종 개정일**: 2026-08-26
- **팀 구성 체제**: **6인 전문 엔지니어링 체제 (AI / DB / INFRA / BE / FE / APP)**
- **마일스톤 목표**:
  - **1단계 (2026.08.27 ~ 2026.09.04)**: 상용 웹 완결 & 375px 모바일 반응형 웹 100% 프로덕션 런칭
  - **2단계 (2026.09.05 ~ 2026.09.11)**: React Native 크로스플랫폼 앱 완결 & Google Play / Apple App Store 출시 제출

---

## 📑 목차

1. [Executive Summary & 서비스 비전](#1-executive-summary--서비스-비전)
2. [서비스 6대 핵심 원칙](#2-서비스-6대-핵심-원칙)
3. [핵심 타깃 사용자 & 페르소나](#3-핵심-타깃-사용자--페르소나)
4. [여행 3단계(전·중·후) 사용자 여정](#4-여행-3단계전중후-사용자-여정)
5. [12대 핵심 기능 상세 명세](#5-12대-핵심-기능-상세-명세)
   - 5.1 온보딩 및 실시간 전역 언어 스위처 (KO / EN)
   - 5.2 1화면 1질문 단계별 취향 카드 위저드 (Taste Step Wizard)
   - 5.3 먹고 싶은 한국 음식(8종 칩) 중심 탐색 & 일정 앵커링
   - 5.4 10개 맞춤형 코스 테마 카테고리
   - 5.5 📷 카메라 메뉴판 OCR 번역기 (알레르기/돼지고기/맵기 분석)
   - 5.6 🎙️ 양방향 실시간 동시 음성 통역기 (STT/TTS 워키토키)
   - 5.7 🗣️ 여행 준비 및 부산 대표 사투리 표현 익히기 위젯
   - 5.8 🏛️ 장소별 한국어 말하기 모달 (관광/식당/택시/숙소 4대 탭)
   - 5.9 카카오맵 리뷰 평점·후기 연동 & 현실적 예상 비용 산출
   - 5.10 날씨 연동 준비물 안내 & 실내 장소 부분 재최적화
   - 5.11 현장 이동 지원 & 외국인 전용 택시 목적지 카드
   - 5.12 트래블 북(Travel Book) & 지도 기반 추억 지도(Memory Map)
6. [추천 및 최단 동선 최적화 알고리즘 (OR-Tools + Spatial Clustering)](#6-추천-및-최단-동선-최적화-알고리즘)
7. [데이터 모델 & 26개 테이블 공간 DB 아키텍처](#7-데이터-모델--26개-테이블-공간-db-아키텍처)
8. [66개 RESTful API 엔드포인트 명세](#8-66개-restful-api-엔드포인트-명세)
9. [6인 팀 역할 분담 및 기술 스택](#9-6인-팀-역할-분담-및-기술-스택)
10. [2단계 마일스톤 개발 계획 (9/4 웹 ➔ 9/11 앱 스토어)](#10-2단계-마일스톤-개발-계획)
11. [크로스플랫폼 배포 전략 & 스토어 심사 통과 가이드](#11-크로스플랫폼-배포-전략--스토어-심사-통과-가이드)
12. [KPI 및 위험 관리 방안](#12-kpi-및-위험-관리-방안)

---

## 1. Executive Summary & 서비스 비전

### 1.1 한 문장 정의
> **LOCAL ROUTE**는 실제 현지인들의 방문 행동과 검증된 카카오 리뷰 평점을 바탕으로, 외국인 관광객이 한국에서 겪는 **탐색 · 이동 · 주문 · 결제 · 소통의 단절을 완벽하게 해소**하고, 수학적 최적화 엔진(OR-Tools)으로 실행 가능한 시간표와 현장 스마트 도구(메뉴판 번역, 음성 통역, 사투리 학습, 택시 카드)를 제공하는 **외국인 현장 완결형 스마트 로컬 여행 플랫폼**이다.

### 1.2 문제 정의 및 해결 솔루션

| 기존 여행 서비스의 한계 | LOCAL ROUTE의 해결책 |
|---|---|
| **구글맵의 한국 내 대중교통/도보 길찾기 단절** | 카카오 모빌리티 길찾기 연동 + 네이티브 지도 및 턴바이턴 대중교통/도보 안내 |
...
| **비효율적인 동선 낭비와 일정 붕괴** | **Google OR-Tools VRP 솔버** 기반 권역별(동/중/서부산) 군집화 및 이동시간 35% 단축 |
...
1. **데이터와 수학적 제약조건 최적화**: 일정 생성은 단순 생성형 AI 대신 검증된 DB와 Google OR-Tools 최적화기가 시간창(Time Window), 영업시간, 예산 제약을 100% 충족하며 계산한다.
...
6. **세로 흐름의 완결성**: `음식/취향 선택 ➔ 일정 자동 생성 ➔ 현장 이동 ➔ 메뉴판 촬영/음성 통역 ➔ 트래블 북 기록 및 회고`로 이어지는 여행 전 과정을 원스톱으로 지원한다.
...
flowchart LR
    subgraph PRE["1. 여행 전 (Plan & Prep)"]
        A[언어 선택 KO / EN] --> B[1화면 1질문 취향 카드 선택]
    ...
    subgraph DURING["2. 여행 중 (Execute & Assist)"]
        ...
        E --> F[📷 카메라 메뉴판 OCR 번역]
        F --> G[🎙️ 양방향 실시간 음성 통역]
        G --> H[🏛️ 4대 카테고리 한국어 TTS & 택시 카드]
    end

    subgraph POST["3. 여행 후 (Review & Memory)"]
        H --> I[방문 만족도 & 정보 제보]
        I --> J[지도 기반 추억 지도 & 트래블 북]
        J --> K[여행 피드 공유 & 추천 엔진 학습]
    end
```
...
### 5.1 온보딩 및 실시간 전역 언어 스위처 (KO / EN)
- **화면 우측 상단 고정 플로팅 바**: `한국어 | English` 스위처 상시 노출.
- **반응형 상태 동기화 (`uiLang`)**: 언어 변경 시 새로고침 없이 즉시 모든 버튼(`Speak Korean`, `Route on Map`, `Replace Place` 등), 뱃지, 모달 텍스트가 100% 영문/한국어로 리렌더링.
- **관광지명 병기**: 영어 모드 시 `해운대 해수욕장 (Haeundae Beach)` 형태로 한국어와 영문명을 동시 병기하여 현장 표지판 대조 지원.

### 5.2 1화면 1질문 단계별 취향 카드 위저드 (Taste Step Wizard)
- 기존의 복잡한 긴 폼 입력을 탈피하여, 화면당 하나의 핵심 질문에 집중하는 몰입형 캐러셀 카드 UI 채택.
- 여행 기분(Travel Mood), 선호 활동(Activities), 식단/알레르기(Diet/Allergies)를 단계별로 선택.
...
### 5.8 🏛️ 장소별 한국어 말하기 모달 (4대 카테고리 완비)
- **4대 탭 구성**: 🏛️ TOURIST(관광지), 🍽️ RESTAURANT(식당/카페), 🚕 TAXI(이동), 🏨 LODGING(숙소).
- **14종 필수 현장 문장**: 입장권 구매, 사진 요청, 화장실 위치, 안 맵게 요청, 얼음물 요청, 카드 결제, 체크인 등.
- **TTS 음성 재생 & 대형 텍스트**: 현지 직원/기사님께 직접 화면을 보여주거나 음성을 들려줄 수 있는 기능.

### 5.9 카카오맵 리뷰 평점·후기 연동 & 현실적 예상 비용 산출
- **카카오 평점 시각화**: 장소 카드 상단에 `★ 4.5 (카카오 후기 1,230개)` 및 대표 키워드 3종 표출.
- **현실적 카테고리별 비용 산출**: 무료 관광지 0원, 유료 관람 15,000원, 식당 1인 14,000원, 카페 7,000원, 숙소 80,000원~150,000원 정밀 산출.
...
- `부산에서의 3일간의 추억` 디지털 트래블 북 및 SNS 공유 링크 제공.
...
## 8. 66개 RESTful API 엔드포인트 명세

| 라우트 파일 | 엔드포인트 수 | 주요 역할 |
|---|:---:|---|
| `routes/trips.ts` | 18개 | 여행 생성, 일정 조회, 일정 재최적화(Replan), 장소 교체, 시간표 수정 |
| `routes/account.ts` | 8개 | 회원가입, 로그인, 세션 병합, 프로필 수정, **스토어 심사 계정 탈퇴/삭제** |
| `routes/community.ts` | 9개 | GPS 방문 인증, 리뷰 작성, 장소 정보 제보, 로컬 등급 프로필 |
| `routes/social.ts` | 8개 | 트래블 북(스토리) 작성/조회, 피드 공유, 동행자 팔로우, 신고 |
| `routes/experience.ts` | 7개 | 부산 지역 축제/이벤트, 기념품샵, 실시간 날씨 예보 및 준비물 |
| `routes/collaboration.ts` | 6개 | 동행자 실시간 공동 편집, 초대 링크 발급 및 수락 |
| `routes/commerce.ts` | 5개 | 스폰서 광고 노출/클릭 트래킹, 숙소 제휴 예약 링크 연동 |
| `routes/analytics.ts` | 5개 | 사용자 행동 로그 수집, 선호도 통계, 시스템 KPI 지표 |

---

## 9. 6인 팀 역할 분담 및 기술 스택
...
## 11. 크로스플랫폼 배포 전략 & 스토어 심사 통과 가이드

### 11.1 Apple App Store 심사 통과 3대 필수 요건
...
### 11.2 Google Play Console 출시 요건
