...
commerceRouter.post("/businesses/:id/campaigns", async (req, res, next) => {
  try {
    ...
    if (!isAllowedAdCategory(serviceCategory)) return res.status(422).json({ error_code: "AD_CATEGORY_NOT_ALLOWED", allowedCategories: ["LODGING", "RENTAL_CAR", "TRAVEL_INSURANCE", "TAXI", "AIRPORT_TRANSFER"], message: "숙박·렌터카·여행보험·택시·공항이동 서비스만 광고할 수 있습니다." });
