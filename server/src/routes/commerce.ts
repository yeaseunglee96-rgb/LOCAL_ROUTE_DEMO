...
commerceRouter.get("/ads", async (req, res, next) => {
  try {
    const now = new Date();
    const candidates = await prisma.adCampaign.findMany({ where: { status: "ACTIVE", startsAt: { lte: now }, endsAt: { gte: now } }, include: { place: { include: { petPolicy: { select: { allowed: true } } } } }, take: 100 });
    const ranked = rankEligibleAds(candidates, { mode: typeof req.query.mode === "string" ? req.query.mode : undefined, hasPet: req.query.hasPet === "true", language: typeof req.query.language === "string" ? req.query.language : undefined, category: typeof req.query.category === "string" ? req.query.category : undefined });
    res.json(ranked.slice(0, 3).map((candidate) => ({ campaignId: candidate.id, placeId: candidate.place.id, nameKo: candidate.name, nameEn: null, category: candidate.serviceCategory, imageUrl: candidate.place.imageUrl, label: req.query.language === "EN" ? "Sponsored" : "광고", disclosure: req.query.language === "EN" ? "Essential travel service · paid placement, separate from organic recommendations" : "여행 필수 서비스 · 자연 추천과 분리된 유료 노출", adRankScore: candidate.adRankScore })));
...
commerceRouter.post("/businesses/:id/campaigns", async (req, res, next) => {
  try {
    ...
    if (!isAllowedAdCategory(serviceCategory)) return res.status(422).json({ error_code: "AD_CATEGORY_NOT_ALLOWED", allowedCategories: ["LODGING", "RENTAL_CAR", "TRAVEL_INSURANCE", "TAXI", "AIRPORT_TRANSFER", "PET_MOBILITY"], message: "숙박·렌터카·여행보험·택시·공항이동·반려동물 이동 서비스만 광고할 수 있습니다." });
