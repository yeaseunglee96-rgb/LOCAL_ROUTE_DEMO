...
socialRouter.post("/stories", async (req, res, next) => {
  try {
    ...
    const story = await prisma.story.create({ data: { authorSessionId: session.id, placeId: place.id, itineraryId: item?.day.itineraryId ?? null, itineraryItemId: item?.id ?? null, content, imageDataJson: JSON.stringify(sanitizedImages), rating: Number.isInteger(req.body?.rating) && req.body.rating >= 1 && req.body.rating <= 5 ? req.body.rating : null, visibility, visitVerified: !!verified, petTagged: !!req.body?.petTagged, areaLabel: place.address.split(" ").slice(0, 2).join(" "), publishAt, moderationStatus: "PUBLISHED" } });
