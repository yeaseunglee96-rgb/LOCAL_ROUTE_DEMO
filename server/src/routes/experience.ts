...
experienceRouter.get("/events", async (req, res, next) => {
  try {
    ...
    const [festivals, nightMarkets, traditionalMarkets] = await Promise.all([
      prisma.place.findMany({ where: { category: "FESTIVAL", eventStartDate: { lte: to }, eventEndDate: { gte: from } }, include: { petPolicy: { select: { allowed: true, freshnessGrade: true } } }, orderBy: [{ eventStartDate: "asc" }, { localScore: "desc" }] }),
      // Visit Busan에서 상설 야시장 운영 정보를 확인할 수 있는 장소만 노출한다.
      prisma.place.findMany({ where: { nameKo: { in: ["부평깡통시장", "부평깡통야시장"] } }, include: { petPolicy: { select: { allowed: true, freshnessGrade: true } } }, orderBy: { localScore: "desc" } }),
      prisma.place.findMany({ where: { nameKo: { in: ["자갈치시장", "국제시장", "부전시장", "동래시장", "초량전통시장", "해운대전통시장", "수영팔도시장", "구포시장"] } }, include: { petPolicy: { select: { allowed: true, freshnessGrade: true } } }, orderBy: { localScore: "desc" } }),
    ]);
    const festivalEvents = festivals.map((place) => ({ placeId: place.id, eventType: "FESTIVAL", title: place.nameKo, titleEn: place.nameEn, address: place.address, lat: place.lat, lng: place.lng, startDate: place.eventStartDate, endDate: place.eventEndDate, playTime: place.playTime, petAllowed: place.petPolicy?.allowed ?? null, petPolicyFreshness: place.petPolicy?.freshnessGrade ?? null, imageUrl: place.imageUrl, localScore: place.localScore }));
    const nightMarketEvents = nightMarkets.map((place) => ({ placeId: place.id, eventType: "NIGHT_MARKET", title: "부평깡통야시장", titleEn: "Bupyeong Kkangtong Night Market", address: place.address, lat: place.lat, lng: place.lng, startDate: from, endDate: to, playTime: "매일 19:30–24:00 · 방문 전 운영 확인", petAllowed: place.petPolicy?.allowed ?? null, petPolicyFreshness: place.petPolicy?.freshnessGrade ?? null, imageUrl: place.imageUrl, localScore: place.localScore, officialUrl: "https://www.visitbusan.net/kr/index.do?lang_cd=ko&menuCd=DOM_000000202003001000&uc_seq=1861" }));
    const traditionalMarketEvents = traditionalMarkets.map((place) => ({ placeId: place.id, eventType: "TRADITIONAL_MARKET", title: place.nameKo, titleEn: place.nameEn, address: place.address, lat: place.lat, lng: place.lng, startDate: from, endDate: to, playTime: `운영 ${place.openTime}–${place.closeTime} · 점포별 휴무 확인`, petAllowed: place.petPolicy?.allowed ?? null, petPolicyFreshness: place.petPolicy?.freshnessGrade ?? null, imageUrl: place.imageUrl, localScore: place.localScore }));
...
experienceRouter.get("/festivals", async (req, res, next) => {
  try {
    ...
    const festivals = await prisma.place.findMany({
      ...
      include: { petPolicy: { select: { allowed: true, freshnessGrade: true } } },
    ...
    return res.json({ festivals: festivals.map((place) => ({ placeId: place.id, title: place.nameKo, titleEn: place.nameEn, address: place.address, lat: place.lat, lng: place.lng, startDate: place.eventStartDate, endDate: place.eventEndDate, playTime: place.playTime, petAllowed: place.petPolicy?.allowed ?? null, petPolicyFreshness: place.petPolicy?.freshnessGrade ?? null, imageUrl: place.imageUrl, localScore: place.localScore })) });
  } catch (error) { next(error); }
});
...
experienceRouter.post("/itineraries/:id/festivals/:placeId", async (req, res, next) => {
  try {
    ...
    const festival = await prisma.place.findFirst({ where: { id: req.params.placeId, category: "FESTIVAL" }, include: { petPolicy: true } });
...
experienceRouter.get("/weather", async (req, res) => {
  ...
  const value = { date, region, tempMin: 18 + (seed % 4), tempMax: 25 + (seed % 5), rainProbability, sky: rainProbability >= 60 ? "RAIN" : rainProbability >= 30 ? "CLOUDY" : "CLEAR", isEstimate: true, source: "CACHED_DEMO_FORECAST", outdoorWarning: rainProbability >= 60, petHeatWarning: 25 + (seed % 5) >= 29, cachedForSeconds: 3600 };
